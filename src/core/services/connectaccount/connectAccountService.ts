...
const createRenderLink = async (
  ...
  const callApi = () =>
    api.post('/v1/connectaccounts/render', JSON.stringify(request), header);
  return ApiCaller.caller(
    callApi,
    ConnectAccountMapper.fromRenderObject,
...
const linkCustomerAccount = async (
  ...
  const callApi = () =>
    api.post('/v1/connectaccounts/connect', JSON.stringify(request), header);
  await ApiCaller.callerAndForget(callApi, HttpStatusCode.CREATED);
};

export interface IConnectAccountService {
  createRenderLink: (
    renderCreateArgs: ConnectAccountRenderCreateArgs,
    token: ApiAccessToken
  ) => Promise<ConnectAccountRender>;
  linkCustomerAccount: (
    createArgs: ConnectAccountCreateArgs,
    token: ApiAccessToken
  ) => Promise<void>;
}
...
const instance = (api: AxiosInstance): IConnectAccountService => {
  ...
  return {
    createRenderLink: (
      renderCreateArgs: ConnectAccountRenderCreateArgs,
      token: ApiAccessToken
    ) => createRenderLink(api, renderCreateArgs, token),
    linkCustomerAccount: (
      createArgs: ConnectAccountCreateArgs,
      token: ApiAccessToken
    ) => linkCustomerAccount(api, createArgs, token),
  };
};
