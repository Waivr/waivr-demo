...
const env: EnvType = RequiredAttributes.requireNonNullOrElse(import.meta.env.NODE_ENV, 'development') as EnvType;
...
const apiAccessToken = (): ApiAccessToken => {
  ...
  const secret = RequiredAttributes.requireNonBlank(
      import.meta.env.API_TOKEN_SECRET || import.meta.env.VITE_PUBLIC_API_TOKEN_SECRET
  );
...
const geEnvVars = (): EnvironmentVarsMap => {
  ...
  const publicUrl = RequiredAttributes.requireNonBlankOrNonNullDefault(import.meta.env.PUBLIC_URL, '/');
  const waivrAppApi = RequiredAttributes.requireNonBlankOrNonNullDefault(import.meta.env.WAIVR_APP_API || import.meta.env.VITE_PUBLIC_WAIVR_APP_API, 'api/waivr-service');
  return {
    env,
    publicUrl,
    waivrAppApi,
